...
var populationBtn = document.getElementById('populationBtn');
var languageBtn = document.getElementById('languageBtn');
populationBtn.addEventListener('click', function() {
  var sortCountries = countries.sort((a, b) => b.population - a.population);
  var top10Countries = sortCountries.slice(0, 10);
  var total = top10Countries.reduce((a, b) => a + b.population, 0);
  var worldPopulation = countries.reduce((total, country) => total + country.population, 0);
  var world = { name: 'World', population: worldPopulation };
  ...
  for (let i = 0; i < top10Countries.length; i++) {
    ...
    bar.className = 'bar';
    var width = (country.population / total) * 100;
...
function createLanguageChart() {
  ...
  for (let i = 0; i < countries.length; i++) {
    ...
    for (let j = 0; j < country.languages.length; j++) {
      var language = country.languages[j];
  ...
  var languageArray = [];
  for (var [language, count] of Object.entries(languageCount)) {
    languageArray.push({ language, count });
  }
  languageArray.sort((a, b) => b.count - a.count);
  var top10Languages = languageArray.slice(0, 10);
  var totalCount = top10Languages.reduce((total, language) => total + language.count, 0);
  chartContainer.innerHTML = ''; 
  for (let i = 0; i < top10Languages.length; i++) {
    var language = top10Languages[i];
    var chartBar = document.createElement('div');
    chartBar.className = 'chart-bar';
    ...
    var bar = document.createElement('div');
    bar.className = 'bar';
    var width = (language.count / totalCount) * 100;
    bar.style.width = `${width}%`;
    var value = document.createElement('div');
...
languageBtn.addEventListener('click', createLanguageChart);
